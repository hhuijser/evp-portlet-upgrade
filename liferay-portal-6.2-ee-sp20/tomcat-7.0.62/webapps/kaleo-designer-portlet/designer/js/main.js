		...
		var KaleoDesigner = A.Component.create(
			{
				...
				prototype: {
					...
					isValidDefinition: function(definition) {
						var instance = this;

						var doc = A.DataType.XML.parse(definition);

						return !isNull(doc) && !isNull(doc.documentElement) && !A.one(doc).one('parsererror');
					},
					...
					showEditor: function() {
						...
						if (!content || !instance.isValidDefinition(content)) {
		...
		var AssignmentsEditor = A.Component.create(
			{
				...
				prototype: {
					...
					_compareAssignmentsType: function(assignmentsType1, assignmentsType2) {
						var retVal = 0;

						if (assignmentsType1.label < assignmentsType2.label) {
							retVal = -1;
						}
						else if (assignmentsType1.label > assignmentsType2.label) {
							retVal = 1;
						}
					...
					_valueAssignmentsType: function() {
						...
						var assignmentsTypes = instance._valueNotificationAssignmentsType();
						...
						return assignmentsTypes.sort(instance._compareAssignmentsType);
					},

					_valueNotificationAssignmentsType: function() {
						var instance = this;

						if (!this.notificationAssignmentTypes) {
							var strings = instance.getStrings();

							this.notificationAssignmentTypes = [
								{
						...
						return this.notificationAssignmentTypes;
					}
				}
			}
		);
		...
		var NotificationRecipientsEditor = A.Component.create(
			{
				...
				prototype: {
					_valueAssignmentsType: function() {
						...
						var assignmentsTypes = NotificationRecipientsEditor.superclass._valueNotificationAssignmentsType.apply(instance, arguments);
