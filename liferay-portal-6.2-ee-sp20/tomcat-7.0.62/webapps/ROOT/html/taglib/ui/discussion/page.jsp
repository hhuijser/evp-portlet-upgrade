									...
									<div class="lfr-discussion-controls">
										...
										<c:if test="<%= !hideControls && !TrashUtil.isInTrash(message.getClassName(), message.getClassPK()) %>">
											<ul class="lfr-discussion-actions">
												<c:if test="<%= MBDiscussionPermission.contains(permissionChecker, company.getCompanyId(), threadGroupId, permissionClassName, permissionClassPK, userId, ActionKeys.ADD_DISCUSSION) %>">
							...
							<div class="lfr-discussion-posted-on">
								<c:choose>
									...
									<c:otherwise>
										...
										<liferay-util:buffer var="buffer">
											...
											<span id="lfr-discussion-reply-user-info">
												<div class="lfr-discussion-reply-user-avatar">
													<img alt="<%= HtmlUtil.escapeAttribute(parentMessage.getUserName()) %>" class="user-status-avatar-image" src="<%= UserConstants.getPortraitURL(themeDisplay.getPathImage(), true, imageId) %>" width="30" />
												</div>
										...
										sb.append(HtmlUtil.escapeAttribute(buffer));
										sb.append("\"data-metaData=\"");
										sb.append(HtmlUtil.escapeAttribute(HtmlUtil.escape(parentMessage.getBody())));
										sb.append("\">");
