...
if (layoutRevision.isHead()) {
	taglibHelpMessage = LanguageUtil.format(pageContext, "this-version-will-be-published-when-x-is-published-to-live", layoutSetBranchName, false);
}
	...
	<c:if test="<%= !hasWorkflowTask %>">
		<c:if test="<%= !layoutRevision.isHead() && LayoutPermissionUtil.contains(permissionChecker, layoutRevision.getPlid(), ActionKeys.UPDATE) %>">
			...
			if (layoutRevision.isIncomplete()) {
				label = LanguageUtil.format(pageContext, "enable-in-x", layoutSetBranchName, false);
			}
			else {
				if (workflowEnabled) {
