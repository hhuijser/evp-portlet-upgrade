...
>
	<aui:fieldset>
		...
		<aui:select label="scope" name="<%= displayTerms.SELECTED_GROUP_ID %>" showEmptyOption="<%= false %>">
			...
			<aui:option label="<%= themeDisplay.getScopeGroupName() %>" selected="<%= (displayTerms.getSelectedGroupId() == scopeGroupId) || (displayTerms.getSelectedGroupId() == displayTerms.getLiveGroupId()) %>" value="<%= scopeGroupId %>" />
		</aui:select>
	</aui:fieldset>
</liferay-ui:search-toggle>