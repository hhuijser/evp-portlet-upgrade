...
<aui:fieldset>
	<aui:input autocomplete="off" label="user-name" name='<%= "settings--" + PropsKeys.GOOGLE_APPS_USERNAME + "--" %>' type="text" value="<%= googleAppsUsername %>" wrapperCssClass="lfr-input-text-container" />

	<aui:input autocomplete="off" label="password" name='<%= "settings--" + PropsKeys.GOOGLE_APPS_PASSWORD + "--" %>' type="password" value="<%= googleAppsPassword %>" wrapperCssClass="lfr-input-text-container" />
</aui:fieldset>