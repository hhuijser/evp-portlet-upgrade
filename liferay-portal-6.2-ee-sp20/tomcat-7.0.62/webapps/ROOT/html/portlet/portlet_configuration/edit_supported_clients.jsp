...
<aui:form action="<%= editSupportedClientsURL %>" method="post" name=">fm">
	...
	%>
		...
		<aui:fieldset label='<%= LanguageUtil.get(pageContext, "portlet-mode") + ": " + LanguageUtil.get(pageContext, curPortletMode) %>'>
			...
			<aui:input label="mobile-devices" name="<%= mobileDevicesParam %>" type="checkbox" value="<%= GetterUtil.getBoolean(portletPreferences.getValue(mobileDevicesParam, String.valueOf(mobileDevicesDefault))) %>" />
		</aui:fieldset>
